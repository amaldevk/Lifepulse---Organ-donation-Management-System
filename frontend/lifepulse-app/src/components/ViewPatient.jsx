...
const ViewPatient = () => {
    ...
    const readValues=()=>{
        axios.post("http://localhost:3001/api/patients/view",input).then((response)=>{
            setOutput(response.data)

        })
    }
    const handleback = () => {
        navigate("/patreg"); // Replace "/another-page" with the actual path to the desired page
      };
    useEffect(() => { readValues() }, [])
    const deletepost=(id)=>{
        let data={"_id":id}
        axios.post("http://localhost:3001/api/patients/delete",data).then((response)=>{
  ...
  return (
    <div>
        ...
        <div className="d-flex justify-content-between align-items-center container">
            <h1 className="m-0"><b>Registered Patients</b></h1>
